...
\end{definition}
If the loss  $f$ is convex and smooth then gradient descent converges to a minimum $\theta^*$, assuming some conditions on the step-size. 
...
This means that its gradient is not Lipschitz.
This kind of log-barrier objectives often came up in interior point methods for solving constrained convex problems.
...
\begin{align}
    \min_\theta \ g(\theta) := \expect[x\sim \ptrue]{f(\theta, x)} \; .
\end{align}
...
Perhaps the most classic set of assumptions to prove the convergence of SGD is that 
\begin{itemize}
