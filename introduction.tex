...
\alignn{
	\expect[(x, y)\sim\ptrue]{\loss(y, f_\theta(x))} \; .
	\label{eq:risk-function}
} 
...
%
%Note that for differentiable functions, strong convexity can be equivalently expressed with the gradient.
%\begin{proposition}[strong convexity]
%	A differentiable function $f:\Theta \rightarrow\real$ is $\mu$-strongly convex if and only if its gradient verifies
%	\alignn{
%		\forall \theta, \nu \in \Theta,
%		\mu  \norm{\theta- \nu}^2
%		\leq \langle \nabla f(\theta) - \nabla f(\nu), \, \theta- \nu \rangle \; .
...
OEG enjoyed a significant success on many problems, but it is not until the breakthrough work of \citet{roux2012stochastic} that the true power of the finite sum structure was revealed.
